	...
	else if(showHud)
	{
		...
		if (type)
		{
			...
			if (showDebug)
			{
				...
				sampleInfo << ", Temp: " << (sample.particle.temp * 9 / 5 - 459.67) << " F";
				...
				if (type == PT_CRAY || type == PT_DRAY || type == PT_EXOT || type == PT_LIGH || type == PT_SOAP || type == PT_TRON || type == PT_VIBR || type == PT_VIRS || type == PT_WARP || type == PT_LCRY || type == PT_CBNW || type == PT_TSNS || type == PT_DTEC || type == PT_LSNS || type == PT_PSTN || type == PT_LDTC || type == PT_VOC || type == PT_FIRE)
		...
		if (showDebug)
		{
			...
			if (c->GetAHeatEnable())
				sampleInfo << ", AHeat: " << sample.AirTemperature * 9 / 5 - 459.67 << " F";
