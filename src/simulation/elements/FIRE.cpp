		...
		for (ry=-2; ry<3; ry++)
			if (BOUNDS_CHECK && (rx || ry))
			{
				...
				if ((rt==PT_COAL) || (rt==PT_BCOL))
				{
					...
					{
						if (parts[ID(r)].life> 999 && RNG::Ref().chance(1, 500)) {
							parts[ID(r)].life = 999;
						}
					}
					else if (t==PT_LAVA)
				...
				if ((rt == PT_WOOD))
				{
					if ((t == PT_FIRE || t == PT_PLSM))
					{
						if (parts[ID(r)].life > 499 && parts[ID(r)].temp >= 573.15 && RNG::Ref().chance(1, 500)) {
							parts[ID(r)].life = 499;
						}
					}
				}
				if (rt == PT_WOOD && parts[t].life <= 499 && RNG::Ref().chance(1, 1000)) {
					parts[ID(r)].temp += 250 + parts[ID(r)].temp;
					parts[ID(t)].temp = parts[ID(r)].temp;
				}
				if (rt == PT_COAL && parts[t].life <= 999 && RNG::Ref().chance(1, 2000)) {
					parts[ID(r)].temp += 350 + parts[ID(r)].temp;
					parts[ID(t)].temp = parts[ID(r)].temp;
				}
				if (parts[i].ctype == PT_WOOD && parts[i].life >= 140)
				{
					parts[i].temp = parts[i].temp + RNG::Ref().between(1, 3);
				}
				if (parts[i].ctype == PT_COAL && parts[i].life >= 140)
				{
					parts[i].temp = parts[i].temp + RNG::Ref().between(1, 3);
				}
				if (t == PT_LAVA)
...
static void create(ELEMENT_CREATE_FUNC_ARGS)
{
	...
	if (sim->parts[i].tmp2 == 1) {
		sim->parts[i].temp = sim->parts[i].temp + 1;
	}
}
